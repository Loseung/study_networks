...
success_c=$(cat "$path" | grep 'success' | wc -l)
fail_c=$(cat "$path" | grep 'failed' | wc -l)

suc_p=$(( 100 * $success_c / $total_c ))

echo -e "=== 네트워크 연결 분석 결과 === \n\n 전체 연결 시도: ${total_c}건 \n\n 성공: ${success_c}건 \n\n 실패: ${fail_c}건 \n\n 성>공률: ${suc_p}%"



[skp@192.168.0.41 ~/quests/quests_lhs]$ bash connections_info.sh 
=== 네트워크 연결 분석 결과 === 
